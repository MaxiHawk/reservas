...
Modalidad (propiedad "Modalidad" en Notion):
  - TODAS las reservas son en PAREJA: requieren 2 nombres. La app escribe
    Modalidad = "Pareja" al sellar. La defensa individual la agenda el docente
    manualmente en Notion (un bloque con Estado = "Bloqueado").
...
class Block:
    ...
    def reservable(self) -> bool:
        """Reservable = bloque de defensa Disponible (no es config ni bloqueado).
        Todas las reservas son en pareja, así que ya no depende de la modalidad."""
...
class ReservationService:
    ...
    def reserve(self, block_id: str, estudiante_1: str, estudiante_2: str = "") -> Block:
        ...
        if not e1 or not e2:
            raise ValueError("Las reservas son en pareja: ingresa ambos nombres.")
        ...
        with self._write_lock:  # serializa TODAS las escrituras
            ...
            if block.es_config or block.estado == ESTADO_BLOQUEADO:
            ...
            properties = {
                ...
                PROP_MODALIDAD: {"select": {"name": MODALIDAD_PAREJA}},
                PROP_EST1: {"rich_text": [{"text": {"content": e1}}]},
                PROP_EST2: {"rich_text": [{"text": {"content": e2}}]},
